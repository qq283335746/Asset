    ...
    public class SecurityService : ISecurity
    {
        ...
        public ResResultModel SaveRole(RoleModel model)
        {
            try
            {
                ...
                {
                    if (modelInfo.Named == "Administrators" || modelInfo.Named == "System" || modelInfo.Named == "Users" || modelInfo.Named == "Guest") return ResResult.Response(false, MC.M_SysDataChangedError, "");
        ...
        public ResResultModel GetRolesForUser(string userName)
        {
            try
            {
                ...
                if (roles.Length == 0) return ResResult.Response(false, MC.M_DataEmpty, "");
        ...
        public ResResultModel DelUser(string userName)
        {
            try
            {
                ...
                if (user == null) throw new CustomException(MC.GetString(MC.Params_Data_NotExist, userName));
        ...
        public ResResultModel SaveSiteMulti(SiteMultiModel model)
        {
            try
            {
                ...
                if (effect > 0) return ResResult.Response(true, MC.M_Save_Ok, "");
                else return ResResult.Response(false, MC.M_Save_Error, "");
            }
            catch (Exception ex)
            {
                return ResResult.Response(false, ex.Message, "");
        ...
        public ResResultModel DeleteSiteMulti(string itemAppend)
        {
            try
            {
                ...
                var bll = new SiteMulti();
                if (bll.DeleteBatch(items)) return ResResult.Response(true, MC.M_Save_Ok, "");
                else return ResResult.Response(false, MC.M_Save_Error, "");
            }
            catch (Exception ex)
            {
