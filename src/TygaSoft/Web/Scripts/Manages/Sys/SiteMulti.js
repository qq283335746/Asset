var SiteMulti = {
    ...
    Load: function (pageIndex, pageSize) {
        ...
        var url = Common.AppName + '/Services/SecurityService.svc/GetSiteMultiList';
    ...
    Del: function () {
        ...
        $.messager.confirm('温馨提醒', '确定要删除吗？', function (r) {
            if (r) {
                ...
                var url = Common.AppName + '/Services/SecurityService.svc/DeleteSiteMulti';
                Common.Ajax(url, postData, 'POST', '', true, true, function (result) {
    ...
    Save: function () {
        ...
        var postData = '{"model":{ "Id": "' + Id + '", "Coded": "' + coded + '", "Named": "' + named + '", "SiteTitle": "' + siteTitle + '", "SiteLogo": "' + siteLogo + '", "CultureName": "' + sCultureName + '" }}';
        var url = Common.AppName + '/Services/SecurityService.svc/SaveSiteMulti';
        Common.Ajax(url, postData, 'POST', '', true, true, function (result) {
            $("#dlgSiteMulti").dialog('close');
            jeasyuiFun.show("温馨提示", "保存成功！");
            setTimeout(function () {
    ...
    Cbg: function (cbgId, isLoad, values) {
        ...
        if (!isLoad) {
            ...
            var postData = '{"model":{"PageIndex": 1, "PageSize": 1000 }}';
            var url = Common.AppName + '/Services/SecurityService.svc/GetSiteMultiList';
            Common.Ajax(url, postData, 'POST', '', true, true, function (result) {
                cbg.combogrid({
                    panelWidth: 400,
