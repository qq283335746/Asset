...
namespace TygaSoft.CustomProvider
{
    ...
    public class CustomSiteMapProvider : SiteMapProvider
    {
        ...
        public override SiteMapNode FindSiteMapNode(string rawUrl)
        {
            ...
            var currNode = list.FirstOrDefault(m => m.Url.ToLower().IndexOf(rawUrl.ToLower()) > -1);

            if (currNode == null)
            {
                if (HttpContext.Current.User.Identity.IsAuthenticated)
                {
                    currNode = list.FirstOrDefault(m => m.Title == EnumMenuName.首页.ToString() && !m.Url.Contains(FormsAuthentication.DefaultUrl.Replace("~", "")));
                }
        ...
        public override SiteMapNode GetParentNode(SiteMapNode node)
        {
            ...
            if (parentNode.Title == EnumMenuName.禁止匿名访问.ToString() || parentNode.Title == EnumMenuName.匿名访问.ToString()) return null;
        ...
        protected override SiteMapNode GetRootNodeCore()
        {
            ...
            if (HttpContext.Current.User.Identity.IsAuthenticated)
            {
                rootNode = list.FirstOrDefault(m => m.Title == EnumMenuName.首页.ToString() && !m.Url.Contains(FormsAuthentication.DefaultUrl.Replace("~", "")));
            }
