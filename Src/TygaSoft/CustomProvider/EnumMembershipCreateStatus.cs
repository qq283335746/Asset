...
namespace TygaSoft.CustomProvider
{
    ...
    {
        public EnumMembershipCreateStatus(MembershipUser user)
        {
            if (user == null) throw new ArgumentException(GetStatusMessage(MembershipCreateStatus.InvalidUserName));
            else if (user.IsLockedOut) throw new ArgumentException(MC.Login_AccountLock);
            else if (!user.IsApproved) throw new ArgumentException(MC.Login_AccountAllow);
            else throw new ArgumentException(MC.Login_InvalidPsw);
        }
