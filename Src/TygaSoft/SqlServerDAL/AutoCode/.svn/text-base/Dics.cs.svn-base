...
{
    public partial class Dics : IDics
    {
        #region IDics Member
        ...
        public int Insert(DicsInfo model)
        {
            ...
            sb.Append(@"insert into Dics (AppCode,UserId,DepmtId,ParentId,Coded,Named,Step,Sort,Remark,RecordDate,LastUpdatedDate)
			            values
...
new SqlParameter("@LastUpdatedDate",SqlDbType.DateTime)
        ...
        {
            StringBuilder sb = new StringBuilder(300);
            sb.Append(@"insert into Dics (Id,AppCode,UserId,DepmtId,ParentId,Coded,Named,Step,Sort,Remark,RecordDate,LastUpdatedDate)
			            values
...
new SqlParameter("@LastUpdatedDate",SqlDbType.DateTime)
        ...
        public int Update(DicsInfo model)
        {
            ...
            sb.Append(@"update Dics set AppCode = @AppCode,UserId = @UserId,DepmtId = @DepmtId,ParentId = @ParentId,Coded = @Coded,Named = @Named,Step = @Step,Sort = @Sort,Remark = @Remark,RecordDate = @RecordDate,LastUpdatedDate = @LastUpdatedDate 
			            where Id = @Id
					    ");
...
new SqlParameter("@LastUpdatedDate",SqlDbType.DateTime)
        ...
        public int Delete(Guid id)
        {
            ...
            sb.Append("delete from Dics where Id = @Id ");
            SqlParameter[] parms = {
                                     new SqlParameter("@Id",SqlDbType.UniqueIdentifier)
                                   };
            parms[0].Value = id;
        ...
        public bool DeleteBatch(IList<object> list)
        {
            ...
            {
                n++;
                sb.Append(@"delete from Dics where Id = @Id" + n + " ;");
        ...
        public IList<DicsInfo> GetList(int pageIndex, int pageSize, out int totalRecords, string sqlWhere, params SqlParameter[] cmdParms)
        {
            ...
            sb.Append(@"select * from(select row_number() over(order by Sort) as RowNumber,
			          AppCode,UserId,DepmtId,Id,ParentId,Coded,Named,Step,Sort,Remark,RecordDate,LastUpdatedDate
					  from Dics ");
            if (!string.IsNullOrEmpty(sqlWhere)) sb.AppendFormat(" where 1=1 {0} ", sqlWhere);
            sb.AppendFormat(@")as objTable where RowNumber between {0} and {1} ", startIndex, endIndex);

            IList<DicsInfo> list = new List<DicsInfo>();
        ...
        {
            StringBuilder sb = new StringBuilder(500);
            sb.Append(@"select AppCode,UserId,DepmtId,Id,ParentId,Coded,Named,Step,Sort,Remark,RecordDate,LastUpdatedDate
                        from Dics ");
            if (!string.IsNullOrEmpty(sqlWhere)) sb.AppendFormat(" where 1=1 {0} ", sqlWhere);
