...
namespace TygaSoft.BLL
{
    public partial class OrgDepmt
    {
        ...
        public DataTable GetExportData(string sqlWhere, params SqlParameter[] cmdParms)
        {
            ...
            dt.Columns.Add("所属上级部门编码", typeof(string));
            dt.Columns.Add("所属上级部门", typeof(string));
            dt.Columns.Add("排序", typeof(string));
          
            var orgDepmts = GetList(sqlWhere, cmdParms);
            var rootNode = orgDepmts.FirstOrDefault(m => m.ParentId.Equals(Guid.Empty));
            if (rootNode == null) return dt;
            CreateTreeData(orgDepmts, rootNode.Id, rootNode, ref dt);
        ...
        public void GetStep(IList<OrgDepmtInfo> orgDepmts, Guid id, ref List<Guid> ids)
        {
            if (orgDepmts == null || orgDepmts.Count == 0) return;
            var node = orgDepmts.FirstOrDefault(m => m.Id.Equals(id));
            if (node == null) return;
            if (!ids.Contains(node.Id)) ids.Add(node.Id);
            var parentNode = orgDepmts.FirstOrDefault(m => m.Id.Equals(node.ParentId));
            if (parentNode == null) return;
            if (!ids.Contains(node.Id)) ids.Add(parentNode.Id);
            GetStep(orgDepmts, parentNode.Id, ref ids);
        }
        ...
        public string GetTreeJson(string sqlWhere, params SqlParameter[] cmdParms)
        {
            var jsonAppend = new StringBuilder();
            var list = dal.GetList(sqlWhere, cmdParms).ToList<OrgDepmtInfo>();
        ...
        private void CreateTreeJson(IEnumerable<OrgDepmtInfo> q, object parentId, OrgDepmtInfo rootNode, ref StringBuilder jsonAppend)
        {
            jsonAppend.Append("[");
            var qChild = q.Where(x => x.ParentId.Equals(parentId));
            if (qChild == null || qChild.Count() == 0) qChild = q.Where(x => x.Step.Contains(rootNode.Id.ToString()));
            if (qChild != null && qChild.Count() > 0)
            {
                int index = 0;
